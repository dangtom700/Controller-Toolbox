...
> working starter kit (see [Section 9](#9-starter-kit-whats-already-here)). The roadmap is a proposal for
> whoever owns the effort; the data-contract facts in [Section 4](#4-the-data-contract-matlab-must-honour)
...
- The `MATLAB/+ctrl/` native port (Section 9, Phase 2) is **deprecated / frozen** - kept for reference, no
...
- **First realized study:** [`case-study/Boiler Control MATLAB/`](../case-study/Boiler%20Control%20MATLAB/)
  - a self-contained MATLAB twin of Boiler Control: nonlinear Bell-Astrom plant, all 8 scenarios,
...
- The Tier-1/2/3 framing (Section 5) still applies, but the target is now *native re-simulation per study*
  (Tier 3) built on toolboxes, not a monolithic `+ctrl` re-expression of `lib/`.

Everything below Section 1 is the **original** (pre-verdict) plan; read it as background, but Section 0 supersedes
...
- `MATLAB/` contains only `README.md` (2 lines) and this file. No `.m` code beyond the starter kit
  in [Section 9](#9-starter-kit-whats-already-here).
...
MATLAB/
  ...
  +ctrl/                    # native .m port of lib/ core -> called as ctrl.DiscretePID(...)
    ...
    (more controllers ported incrementally; port-order in Section 6)
...
> should follow the C++ variant (see [Section 5](#5-case-study-analysis-integration-plan)).

**Metric definitions** MATLAB must reproduce (from `compute_metrics`, error `e = ref - y`):

- `iae` = Sigma |e[k]|.Deltat (left Riemann, drops last sample).
...
2. **Runner path.** Passive (a) vs active (b) from [Section 5](#5-case-study-analysis-integration-plan).
...
- **Stale case-study count.** `../CLAUDE.md` (Section 1) says "31 case studies (19 complete, 12 open)", but
...
- [`examples/verify_lqr_kalman.m`](examples/verify_lqr_kalman.m) - asserts the DARE residual approx = 0,
