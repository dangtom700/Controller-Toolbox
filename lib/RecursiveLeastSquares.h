 ...
 * @code
 *   A(q)y[k] = B(q)u[k] + e[k]
 *   y[k] = -a1.y[k-1] - ... - a_nₐ.y[k-nₐ]
 *          + b1.u[k-1] + ... + b_nᵦ.u[k-nᵦ] + e[k]
 * @endcode
 *
 * **Regressor:** phi[k] = [-y[k-1], ..., -y[k-nₐ], u[k-1], ..., u[k-nᵦ]]ᵀ \in ℝ^(nₐ+nᵦ)
 *
 * **Recursive update (exponential forgetting via scalar lambda):**
 * @code
 *   K[k]     = P[k-1].phi[k] / (lambda + phi[k]ᵀ.P[k-1].phi[k])
 *   e[k]     = y[k] - phi[k]ᵀ.theta[k-1]           (a-posteriori prediction error)
 *   theta[k]     = theta[k-1] + K[k].e[k]
 *   P[k]     = (P[k-1] - K[k].phi[k]ᵀ.P[k-1]) / lambda
 * @endcode
 *
 ...
 *
 * @see Astrom & Wittenmark, "Adaptive Control" (1995) Ch. 2.
 * @see Ljung, "System Identification" (1999) Section 11.2.
 */
...
public:
     ...
     * @param lambda   Forgetting factor lambda \in (0, 1].
     *                 - lambda = 1: standard least squares (weights all data equally).
     *                 - lambda < 1: exponential forgetting; effective window approx = 1/(1-lambda) samples.
     ...
     * @return A-posteriori prediction error e[k] = y[k] - phi[k]ᵀ.theta[k-1].
     */
    ...
    /** @brief Current parameter estimate theta = [a1,...,a_nₐ, b1,...,b_nᵦ]ᵀ. */
    const Eigen::VectorXd &params() const { return theta_; }

    /** @brief Monic denominator polynomial [1, a1, ..., a_nₐ] (length nₐ + 1). */
    Eigen::VectorXd denominator() const;

    /** @brief Numerator polynomial [b1, ..., b_nᵦ] (length nᵦ). */
     ...
     * @return Discrete-time TransferFunction in z^-^1 form.
     */
    ...
    /** @brief Forgetting factor lambda. */
    double forgettingFactor() const { return lambda_; }

private:
    int na_, nb_, ntheta_;
    double Ts_, lambda_, P0_scale_;
    Eigen::VectorXd theta_; ///< Parameter estimate theta.
