 ...
 * The augmented state xₐ = [Deltax; y] is propagated with incremental inputs DeltaU, giving a
 * built-in integrating disturbance model:
 * @code
 *   xₐ[k+1] = Aₐ.xₐ[k] + Bₐ.Deltau[k]     Aₐ = [A, 0; C.A, I]
 *   y[k]    = Cₐ.xₐ[k]                  Bₐ = [B; C.B],  Cₐ = [0, I]
 * @endcode
 *
 * **2. Reference trajectory** - the setpoint is approached along a first-order filter:
 * @code
 *   y*[k+j] = alphaʲ.y[k] + (1-alphaʲ).r[k],   alpha \in [0, 1)
 * @endcode
 * - alpha = 0: step reference (same as DiscreteMPC).
 * - alpha -> 1: very soft approach; reduces overshoot without derivative weights.
 *
 ...
 *
 * @note Output constraints are supported via diagonal constraint tightening (see GPCParams::yMin /
 *       yMax). The approximation is exact for the first prediction step and conservative thereafter.
 *       For hard polytopic output constraints across all horizons, use DiscreteMPC instead.
 *
...
namespace ctrl
{
    ...
    struct GPCParams
    {
        ...
        double rho_y = 1.0; ///< Output tracking weight (Qy = rhoy.I).
        double rho_u = 0.1; ///< Move suppression weight (Ru = rhou.I).

        /**
         * @brief Reference trajectory softening factor alpha \in [0, 1).
         *
         * alpha = 0 recovers a step reference (same as DiscreteMPC).
         * alpha -> 1 produces a very soft approach, reducing overshoot without tuning derivative weights.
         */
        double alpha = 0.0;
        ...
        /**
         * @brief Output bounds yMin / yMax enforced via diagonal constraint tightening.
         *
         * For each horizon step j, the constraint yMin <= y_pred[j] <= yMax is mapped to
         * an implied DeltaU[j] box bound using the diagonal of the condensed step-response
         * matrix G: g_jj = Ca * Ba (the first Markov parameter, constant along the diagonal).
         *
         * **Accuracy**: Exact for j=0 (first prediction step). Conservative for j>1 because
         * the off-diagonal coupling terms (earlier DeltaU values shifting y[j]) are ignored.
         * For strong cross-step coupling or MIMO plants (p>1), use DiscreteMPC with explicit
         * output-constraint rows instead.
         *
         * **Infeasibility**: If a bound cannot be satisfied simultaneously with u/du limits,
         * the u/du limits take precedence (the y-bound contribution to lb/ub is silently clipped).
         *
         * Default (+/-1e9): inactive - no output constraint applied.
         */
        double yMin = -1e9; ///< Soft lower bound on predicted output (SISO: scalar).
        double yMax = 1e9;  ///< Soft upper bound on predicted output (SISO: scalar).
    ...
    public:
        ...
        /**
         * @brief Current augmented state xₐ = [Deltax; y] (size n+p).
         *
