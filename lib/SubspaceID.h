 ...
 * 2. Thin LQ decomposition isolates the (3,2) block L32 (oblique projection of Yf onto Wp ⊥ Uf).
 * 3. SVD(L32): singular values reveal system order; left singular vectors form the
 *    extended observability matrix Γ = U[:, :n]·√(S[:n]).
 * 4. Extract C = Γ[:p, :], A from shift-invariance of Γ (least squares).
...
struct SubspaceIDResult
{
     ...
     *
     * Kalman gain K such that x[k+1] = A·x[k] + B·u[k] + K·ε[k].
    ...
    /**
     * @brief Innovation covariance Λ = E[ε·εᵀ] (p × p).
     *
