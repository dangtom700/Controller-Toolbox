...
**Why it matters:** the reflector gain is nonlinear in tilt error — even a 10° deviation from
...
- **Charging mode** (high irradiance): excess solar energy melts the PCM (φ_pcm: 0→1),
  absorbing heat at constant T_melt without raising T_abs, extending the absorber's effective
  heat capacity.
- **Discharging mode** (solar drop or post-sunset): the solidifying PCM (φ_pcm: 1→0) releases
...
|---|------|---------------|----------------|--------------|
| 1 | OpenLoop | — | Fixed tilt theta = 45° | No feedback; baseline showing drift with sun movement and wind losses |
...
| 5 | FuzzyPID | `FuzzyPIDController` | e_max=30°C, de_max=2°C/s | Gain-scheduled for charging plateau (low gain) vs. standard tracking (high gain) |
...
|----|-------------|------------------------|--------------------------|
| s01_clear_sky_tracking | Full day, clear sky; sun angle sweeps 6–18 h; TBPR must track | G = 800 W/m^2, T_a = 30°C, V = 1 m/s | Obj 1: TBPR tracking across day |
| s02_wind_disturbance | Constant sun; wind gusts V = 1→6 m/s at t = 600 s; elevated convective losses | G = 750 W/m^2, V step 1→6 m/s, T_a = 28°C | Obj 2: weather disturbance rejection |
...
| s05_overtemp_protection | Summer noon peak irradiance; must limit T_abs ≤ 280°C while still completing cooking | G = 1050 W/m^2, V = 0.5 m/s | Obj 2: absorber protection constraint |
...
- **PCM plateau:** The latent heat term `m_pcm * lambda_pcm * d(phi_pcm)/dt` creates a thermal plateau near T_melt ≈ 55°C where `dT_abs/dt ≈ 0`. This appears to feedback controllers as a temporary near-zero plant gain. ADRC ESO compensates automatically; PID/MPC designs should be tested through s03 specifically to verify they do not saturate during the plateau.
...
- **Absorber hard constraint:** `T_abs ≤ T_abs_max = 280°C` is safety-critical. MPC/ScenarioMPC enforce it as a hard constraint in the QP. Other controllers clamp shade output manually when T_abs approaches the limit.
