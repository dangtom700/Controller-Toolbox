...
// Robustness analysis for the Solar Cooker case study: WCET profiling, Monte
// Carlo (plant-parameter perturbation), and fault sweep (sensor/actuator/
// setpoint faults). Reuses the real SolarCookerPlant + every controller in
// the roster via a lightweight in-memory simulation loop with no per-step
// file I/O.
//
...
// solar-cooker plant with no per-step file I/O. If wcet_us is non-null,
// records the wall-clock time of every controller.compute() call.
SimSummary runOnce(const cooker::PlantParams& plant_params,
                    cooker::ControllerBase&    ctrl,
                    ...
                    std::vector<double>*       wcet_us = nullptr)
{
    ...
    cooker::SolarCookerPlant plant(plant_params);
    plant.reset(30.0, 30.0);
    ctrl.reset();

    // Nominal clear-sky disturbance + cooking target, matching s01_clear_sky_tracking.json.
    const cooker::Disturbance d_nom{800.0, 30.0, 1.0};
    const double ref_nom = 95.0;
    ...
    for (int k = 0; k < N_steps; ++k) {
        ...
        const double T_ref = robust::applySetpointFault(ref_nom, fault, t);
        final_ref            = T_ref;

        const Eigen::Vector2d& x_true = plant.state();

        // Sensor fault: perturb only the measured T_pot fed to the controller.
        ...
        if (wcet_us) {
            auto t0 = Clock::now();
            f_raw   = ctrl.compute(x_meas, T_ref);
            auto t1 = Clock::now();
        ...
        // Hard safety override (on the TRUE absorber temperature), same as
        // simulation_runner.cpp -- then actuator fault, then clamp.
        if (x_true(0) >= plant_params.T_abs_max - 1.0) f_raw = 1.0;
        const double f_fault = robust::applyActuatorFault(f_raw, fault, t);
        const double f_shade = std::clamp(f_fault, 0.0, 1.0);

        const double error  = T_ref - x_true(1);
        out.iae            += std::abs(error) * dt;
        sum_e2              += error * error;
        out.max_abs_error    = std::max(out.max_abs_error, std::abs(error));
        out.max_u             = std::max(out.max_u, f_shade);
        sum_u                 += f_shade;
        sum_u2                += f_shade * f_shade;
        max_y = std::max(max_y, x_true(1));
        min_y = std::min(min_y, x_true(1));

        if (!std::isfinite(x_true(0)) || !std::isfinite(x_true(1)) || !std::isfinite(f_shade) ||
            x_true(1) > 500.0) {
            out.stable = false;
        }
...
std::vector<std::unique_ptr<cooker::ControllerBase>> buildControllers(const cooker::PlantParams& plant) {
    std::vector<std::unique_ptr<cooker::ControllerBase>> v;
    v.push_back(std::make_unique<cooker::OpenLoopCtrl>(plant));
    v.push_back(std::make_unique<cooker::PIDCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::ADRCCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::MPCCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::FuzzyPIDCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::SMCCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::GainScheduledCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::MRACCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::L1AdaptiveCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::NeuralPIDCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::DynaCookerCtrl>(plant));
    v.push_back(std::make_unique<cooker::ScenarioMPCCookerCtrl>(plant));
    return v;
}

void runWcet(const cooker::PlantParams& plant,
...
void runMonteCarlo(const cooker::PlantParams& plant,
                   std::vector<std::unique_ptr<cooker::ControllerBase>>& controllers,
                   const std::string& study_dir)
{
    ...
    for (auto& ctrl : controllers) {
        ...
        for (int s = 0; s < kNumMcSamples; ++s) {
            cooker::PlantParams p = plant;
            p.eta_box   *= robust::perturbFactor(kMcSigma, rng);
            p.eta_TBPR  *= robust::perturbFactor(kMcSigma, rng);
            p.m_pcm     *= robust::perturbFactor(kMcSigma, rng);
            p.h_abs_pot *= robust::perturbFactor(kMcSigma, rng);
            p.U_loss    *= robust::perturbFactor(kMcSigma, rng);

            SimSummary res = runOnce(p, *ctrl, no_fault, rng);
            if (!res.stable) ++n_unstable;
            iae_values.push_back(res.iae);

            csv << "SolarCooker," << ctrl->name() << ',' << s << ','
...
void runFaultSweep(const cooker::PlantParams& plant,
                   std::vector<std::unique_ptr<cooker::ControllerBase>>& controllers,
                   const std::string& study_dir)
{
    ...
    const std::vector<Trial> trials = {
        {FaultKind::SensorBias,    {2.0, 5.0, 10.0}},    // [degC] T_pot sensor offset
        {FaultKind::SensorNoise,   {1.0, 2.0, 4.0}},     // [degC] T_pot sensor noise sigma
        {FaultKind::ActuatorLoss,  {0.20, 0.50, 0.80}},  // fraction of f_shade lost
        {FaultKind::ActuatorStuck, {0.0, 0.5, 1.0}},     // frozen f_shade
        {FaultKind::SetpointStep,  {5.0, 10.0, 20.0}},   // [degC] T_ref step
    };
    ...
    for (auto& ctrl : controllers) {
        ...
        for (const auto& trial : trials) {
            for (double mag : trial.magnitudes) {
                ...
                csv << "SolarCooker," << ctrl->name() << ',' << robust::faultKindName(trial.kind) << ','
...
int main(int argc, char* argv[])
{
    ...
    try {
        plant = cooker::PlantParams::fromJson(cfg_path);
    ...
    std::cout << "Solar Cooker with Reflector and Absorber - Robustness Analysis\n";
    std::cout << "==================================================================\n";
