...
| 1     | θ           | Body pitch angle [rad]                   |
| 2     | φ           | Body roll angle [rad]                    |
...
```
M q̈ + C q̇ + K q = B_u z_r + B_f F_act
```
...
```
F_spring = k_s (Z + a_i θ − (W/2)φ − Z_wri)
```
Left wheel i at `−W/2`:
```
F_spring = k_s (Z + a_i θ + (W/2)φ − Z_wli)
```
...
| I_x       | 600 kg·m² | Roll inertia |
| I_y       | 1200 kg·m² | Pitch inertia |
...
```
delay_mid  = L1 / v_vehicle   ≈ 90 ms
delay_rear = (L1+L2) / v_vehicle ≈ 180 ms
...
| 10 | ADRCCtrl | Per-wheel ADRC, ω_o=80, ω_o·Ts=0.40<0.5 | |
...
Bounds: Kp ∈ [100, 20000], Kd ∈ [10, 2000]  (same for all 6 wheels)
```
...
- **ADRC:** ω_o = 80, Ts = 0.005 s → ω_o · Ts = 0.40 < 0.5 (stability constraint satisfied).
