...
|-------------------|------------------|------------------|
| Roll            | x1               | Roll angle theta   |
| Pitch           | x2               | Pitch angle ϕ  |
| Surge (forward motion) | x3      | Horizontal position s |

**Key states**: $x=[theta,\;ϕ,\;s]^T$ with units rad, rad/s, m respectively.  
...
\begin{aligned}
\dot s &= v \cos phi,\\
\dot phi &= \omega \sin theta,\\
\dot theta &= \frac{\tau_{\text{roll}}}{I_z},\\[4pt]
M(\theta,\varphi)\,\ddot u + D(\theta,\varphi)\,\dot u + C(u) &= T_{\text{pitch}}+T_{\text{roll}},\\
M(\theta,\varphi)\,\ddot phi + D(\theta,\varphi)\,\dot phi + \underbrace{\frac{D_w}{I_z}}_{\text{wave-induced}} &= -F_{w,phi} - F_{g,phi},\\[4pt]
T_{\text{pitch}}+T_{\text{roll}} &= J_{p}\,\ddot phi + J_{r}\,\ddot theta .
\end{aligned}
$$
...
|--------|--------------|---------|
| $I_z$ | 1.2 * 10⁶ kg.m^2 | Surge inertia |
| $J_p$ | 3.5 * 10^4 kg.m^2 | Pitch moment of inertia |
| $J_r$ | 4.0 * 10^4 kg.m^2 | Roll moment of inertia |
...
M(\theta,\varphi)\,\ddot u + D(\theta,\varphi)\,\dot u + C(u) 
   ...
   - k_w \sin(omegat)\,\frac{\partial s}{\partial x},
$$


   where the term $k_w \sin(omegat)\,\partial s/\partial x$ models periodic wave excitation.
...
| Actuator saturation | Linear torque limit $|T_{\text{pitch}}+T_{\text{roll}}| <= T_{\max}=1500$ N.m/s^2. | Must be enforced in MPC feasibility constraints. |
...
1. **Simple / Baseline - PID**  
   *When it works*: In steady-state disturbance rejection when $|theta|$ and $|phi|$ remain small ($<10$^\circ).  
...
2. **Linear State-Space - LQR (or Tuned LQR)**  
   ...
   *Why suitable*: Provides optimal gain for tracking reference speed while respecting torque limits; can be extended to include disturbance observer (LQE) that estimates wave excitation $k_w\sin(omegat)$.
...
- The paper presents an **LQR** tuned via pole placement, achieving decent speed regulation yet showing noticeable overshoot when simulating realistic wave amplitudes (approx =0.5 rad).  
...
| S2 | Moderate wave excitation ($k_w=200$ N/(rad.s), period 8 s) | Wave amplitude $\pm0.15\sin(pit/4)$. |
...
|--------|--------------------------|-----------------------|
| IAE (Integral of Absolute Error) | $\displaystyle\int_0^{T}|x_{ref}-x|\,dt$ for $theta,\varphi,s$ | < 5^\circ roll error; < 1.2^\circ pitch error |
...
- **MPC (default horizon 15 s)** achieved < 0.12^\circ roll error in S2, with only a single torque-limit event during the simulated wave peak.
- **PID** exhibited ~30 % overshoot and sustained residual angle deviation (> 0.5^\circ) even after wave disturbances were introduced.
