...
The paper studies a **marine vessel steering dynamics** modeled as a planar under‑actuated vehicle with two degrees of freedom: heading angle θ and longitudinal position ξ. The state vector is:

| Index | Symbol | Description | Unit |
|-------|--------|-------------|------|
| 0 | θ (yaw) | Heading / orientation of the vessel | rad |
| 1 | ν̇ (steering rate) | Rate of change of heading angle | rad/s |
| … | ΔF (thrust‑vectoring force) | External longitudinal thrust from rudders/hydrofoils | N |
...
|-----------|--------|-------------------|
| Mass (vehicle + water ballast) | $M$ | 1.2 × 10⁵ kg |
| Damping coefficient (longitudinal) | $K_d$ | 4.5 × 10³ N·s/m |
| Wave excitation amplitude | $K_w$ | 0.12 m |
| Wave frequency | $\omega$ | π rad/s (≈1 Hz) |
...
|------------|-------------|
| Linear small‑angle approximation for $f_{\text{hydro}}$ (valid when |$θ|$| ≪ 1 rad) |
...
3. **Nonlinear Optimisation – Model Predictive Control (MPC)**  
   - Recommended for handling cross‑wave disturbance, heading constraint ($|θ|\le π/4$), and actuator saturation.  
   - Provides receding horizon optimality and can enforce terminal constraints (e.g., final angle tracking).

4. **Robust / Adaptive – Sliding Mode or H∞**  
...
| S3 | High speed maneuvering with limited steering authority ($|u|\le 5000$ N) | $v=15$ m/s, reference heading change $\Deltaθ_{ref}=π/6$ rad in 10 s |
...
|--------|--------------------------|
| IAE (Integral of Absolute Error) for heading | $\displaystyle\text{IAE}_Θ=\int_0^{t_f}|θ(t)-θ_{ref}(t)|dt$ |
| Settling time to 95 % tracking accuracy | Time $t_{95}$ where $|\theta(t_{95})-θ_{ref}|\le0.05\,π/6$ rad |
...
- **MPC + sliding mode** achieves < 5° heading error after 5 s sea excitation, while PID would diverge.  
- Settling time for full heading change is ~12 s (MPC) vs. > 20 s (PID).  
- Control effort peaks at ≈ 4400 N (≤ saturation), indicating effective handling of actuator limits.  
...
- **Linearisation assumptions**: The controller assumes small‑angle dynamics; large turns (> 45°) may require a higher‑order model or adaptive gain.  
- **Hydrodynamic parameter variations**: Uncertainty in lift/drag coefficients could be modeled with an H∞/robust MPC for extended reliability under varying hull condition.  
