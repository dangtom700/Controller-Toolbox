"""
main.py - Satellite Launch Vehicle (SLV) pitch-plane attitude case study.

Runs all 12 controllers x 5 scenarios, writes CSV telemetry to ../logs/, and
prints a summary. Auto-discovered by run.py Phase 7.
...
def main():
    ...
    print("=" * 64)
    print("  Satellite Launch Vehicle Systems - pitch-plane attitude control")
    ...
    for sc in scenarios:
        for c in controllers:
            ...
            try:
                ...
                print(f"  [{done:>3}/{total}] {sc['id']:26s} | {c.name():18s} "
                      f"IAE={res['IAE']:9.4f}  maxErr={res['max_error']:7.4f}")
            except Exception as exc:
                print(f"  [{done:>3}/{total}] {sc['id']:26s} | {c.name():18s} ERROR: {exc}")
...
# ===========================================================================
# Analysis hooks - tools/monte_carlo.py, tools/fault_sweep.py, tools/wcet_report.py
...
def run_wcet_profile(log_dir=None):
    ...
    scenario; writes one logs/wcet_{scenario_id}.csv per scenario."""
