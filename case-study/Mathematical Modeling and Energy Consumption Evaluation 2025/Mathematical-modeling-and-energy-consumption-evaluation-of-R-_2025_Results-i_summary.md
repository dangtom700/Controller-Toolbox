...
| $J_{zz}$ (second moment of inertia) | 2.5 * 10⁶ kg.m^2 | From CAD model |
...
| **5** - Robust / Adaptive (e.g., Hinf, Tube MPC) | Not explicitly required by current linear analysis but advisable if external disturbances vary in magnitude or if actuator gains are uncertain (e.g., thruster thrust variance). Provides worst-case performance guarantees against model errors and sensor noise. |
...
- With the 2-step MPC, heave settles in approx = 0.45 s (vs. 0.65 s with PID) and pitch to < 5^\circ within 1 s.
...
- **Adaptation:** Implementation of adaptive MPC or Hinf schemes would provide guarantees against model uncertainties (e.g., thruster thrust variation, environmental changes).
