...
The paper models a **reconfigurable marine surface platform** equipped with thrusters and a payload of variable mass distribution. The plant has three degrees of freedom in horizontal motion: surge translation (**x**), sway translation (**y**), and roll attitude (**θ**). Additional states include thrust vector angles (**α**, β) for directional control.
...
| 1     | y      | Sway position                  | m             |
| 2     | θ      | Roll angle (pitch about vertical) | rad           |
| 3     | α      | Thrust azimuth angle            | rad            |
| 4     | β      | Thrust elevation angle          | rad            |
...
- Payload mass imbalance (**Δm**) causing an additional roll moment (**M_{imb}**).  
...
| $I$   | Inertia about roll axis   | 5,200 kg·m² |
...
- Thrust magnitude $|T|$ ≈ 0.3·$T_{\max}$ (to keep nonlinear thrust‑torque terms within 10%).  
- Roll angles $|\theta|$ < 15° to apply small‑angle approximations.  
...
- **Implementation details:**
  - Horizon ≈ 2–3 s to capture wave influence.  
...
- H∞ Control: Useful for worst‑case disturbance analysis, yet the problem’s dominant uncertainty is stochastic wave forcing which MPC handles naturally.
...
- **SLM** prevented overshoot in S3 despite payload imbalance; residual oscillations (<2°) remained due to model prediction horizon limits.
...
- **Assumption of small‑angle dynamics** may degrade performance if platforms operate near pitch angles > 20°; future work could incorporate full nonlinear roll dynamics.
