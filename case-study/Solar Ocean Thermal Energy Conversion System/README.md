...
|--------|-------------|------|
| `T_h(t)` | Hot-side working fluid / evaporator temperature | ^\circC |
| `T_c(t)` | Cold-side condenser temperature | ^\circC |
| `T_coll(t)` | Solar collector outlet temperature | ^\circC |
...
| Solar hot water temperature | T_h | 54-72 ^\circC | Tested range; drives superheating degree DeltaT_Super |
| Cold sink temperature | T_c | ~4-10 ^\circC | Simulated deep ocean water |
| Temperature differential | delta_T | ~44-68 ^\circC | Primary ORC driving force |
| Superheating degree | DeltaT_Super | 0-18 ^\circC | T_h above saturation; broadens expander enthalpy drop |
| Expander inlet pressure limit | P_inlet_max | **1.38 MPa** | Hard mechanical safety constraint on scroll expander |
| Nominal ORC thermal efficiency | eta_th | 0.04-0.10 | Low-grade heat; raised >150% vs. non-solar OTEC by superheating |
| Control actuator 1 | m_dot_wf | via VFD | Working fluid pump speed; primary power-maximisation variable |
| Control actuator 2 | m_dot_f | via VFD | Solar collector loop pump; regulates T_h and DeltaT_Super |
...
- **eta_th, eta_ele, eta_ex** - thermal, electrical, and exergy efficiencies
...
### 3 - Optimise Superheating Degree (DeltaT_Super) via Solar Heat Regulation
...
entering the expander. The paper tests solar hot water temperatures from **54^\circC to 72^\circC**.

**Effect:** raising T_h broadens the specific enthalpy drop Deltah_exp across the expander and
...
**Control objective:** dynamically adjust solar heat delivery to track the DeltaT_Super that
...
| 5 | LQR | `DiscreteLQR` | Q=diag(100,100,1), R=diag(1,0.1) | States {T_h, T_coll, T_c}; inputs {m_dot_f, m_dot_wf}; linearised at mid-range T_h=63^\circC |
| 6 | FuzzyPID | `FuzzyPIDController` | e_max=18^\circC, de_max=0.5^\circC/s | Handles nonlinear ORC efficiency curve; softer gain near P_inlet limit |
...
| 9 | GainScheduled | `GainScheduledController` | Schedule on T_h brackets: <58^\circC, 58-66^\circC, >66^\circC | Matches the three operating regions tested in paper (54, 63, 72^\circC hot-water temperatures) |
...
|----|-------------|------------|--------------------------|
| s01_mppt_steady | Steady clear-sky; track W_net optimum by co-adjusting m_dot_f and m_dot_wf | G=800 W/m^2, T_c=6^\circC, T_h_range=54-72^\circC | Obj 1: MPPT steady-state optimisation |
| s02_solar_step | T_h steps 54->72^\circC (high solar superheating); must raise m_dot_wf without breaching 1.38 MPa | T_h step, fixed cold source | Obj 2 + Obj 3: pressure-constrained superheating |
...
| s04_low_delta_T | Ocean sink warms to 20^\circC; reduced delta_T; must maintain P_inlet within limits at lower T_h | Reduced cold source | Obj 1: efficiency maximisation at reduced driving force |
...
- **Three operating regions from paper:** T_h approx = 54^\circC, 63^\circC, 72^\circC were the tested hot-water temperatures. GainScheduled uses these as bracket boundaries. NeuralPID and DynaCtrl learn across the full 54-72^\circC range.
- **Efficiency reference:** The paper reports eta_th rising by >150% when solar superheating is used vs. non-solar OTEC on the same cold source. This is the quantitative benchmark a simulation controller should aim to match or exceed.
