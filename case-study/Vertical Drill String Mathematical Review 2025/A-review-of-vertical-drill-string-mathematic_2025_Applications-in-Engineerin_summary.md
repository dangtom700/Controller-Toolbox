...
|-------|--------|-------------|------|
| 0 | η₁, η₂ | Longitudinal positions of the two hulls (head and stern) | m |
| 1 | ν₁, ν₂ | Linear velocities of the respective hulls | m/s |
| … | ψ₁, ψ₂ | Roll angles of each hull about its longitudinal axis | rad |

**Inputs:**  
- $u$ – rudder angle (‑15° … +15°).  
...
|--------|---------------|------------------|
| $J_{\text{roll}}$ | 2.5 × 10⁴ kg·m² | Roll inertia matrix per hull |
| $C_d$ | 1.0 × 10³ Nm²/s² | Quadratic damping coefficient (wind‑wave) |
| $b$ | 3.0 × 10² Nm·s/m | Viscous roll‑damping constant |
| $k_T$ | 1.2 × 10⁴ Nm/rad | Linear thrust coefficient |
| $k_2$ | 4.5 × 10³ Nm/rad³ | Cubic gain for nonlinearity |
| $J_{\text{prop}}$ | 1.0 × 10⁵ kg·m² | Propeller inertia constant |
| $D_{\text{shaft}}(\omega)$ | Proportional + low‑pass filter (ω₀≈30 rad/s) | Shaft viscous friction |
...
3. **Assumptions**  
   ...
   - Actuator is saturated at $\pm15^\circ$ (≈ ±0.26 rad).  
...
| S3 | High‑speed maneuvering with aggressive throttle, producing large roll excitation (> 15°). | Rudder commanded at saturation (+15°); external disturbance increased to F_max; reference heading change in 5 s. |
...
| Settling Time $t_s$ | Time to reach 1 % of reference within ±5° (for small‑angle tolerance). | Identify time when $|\psi_i(t)-\psi_{ref}(t)|<0.05^\circ$. |
| Control Effort (Nm) | Integral of rudder torque magnitude over simulation horizon. | $\displaystyle U = \int_{0}^{T}|u(t)|dt$ |
| Saturation Events | Number of times rudder exceeds ±15° during maneuver S3. | Count spikes in $|u|$. |
...
- **NMPC** achieves < 0.02° roll overshoot in S3, vs. > 0.15° for PID (saturation‑induced).
