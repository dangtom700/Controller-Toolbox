...
**Description:** The study focuses on a **four-degree-of-freedom marine platform** used for wave energy conversion. Each degree of freedom corresponds to one translational motion (x, y) and two rotational motions about orthogonal axes (theta1, theta2). The plant consists of:
...
|-----------|--------|---------------|------|
| Mass       | $m$ | 1,500        | kg   |
| Inertia    | $I_1, I_2$ | 10^4, 5 * 10^3 | kg.m^2 |
...
- **Limitations:** Breakdown at high excitation frequencies or large roll angles (>5^\circ).
...
| S3 | Actuator saturation (maximum force/torque limit of +/-200 N.m) | Apply step disturbance $F_w = -300\text{ N}$ during roll >5^\circ |
| S4 | Deep stall condition (large roll angle) to assess NMPC constraints | Roll reference set at 10^\circ, system dynamics altered by high-angle nonlinear terms |
