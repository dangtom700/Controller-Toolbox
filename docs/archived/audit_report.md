...
Every `computeRef()` call (the real-time control step) allocates `x_traj` (a `std::vector<VectorXd>` of size Np+1), `A_list` (size Np), and `B_list` (size Np) on the heap, plus full `MatrixXd` temporaries for each Jacobian call. For Np=20 and n=5 this is O(Np * n^2) allocations per control cycle at the real-time rate.
...
**Fix:** Add a `[zero_phase]` Catch2 test: drive a known-frequency sinusoid through the filter and assert the peak-to-peak phase error is below 1^\circ.
...
Existing tests only verify that outputs are finite. No test confirms the ESC actually locates the optimum of a known cost function such as `J(theta) = (theta - 2)^2` within tolerance after N steps.

**Fix:** Add an `[extremum_seeker]` convergence test with a closed-form quadratic cost and an assertion on the final `theta` estimate.
...
`///< Normalised log-sum-exp weights (N)` on `Eigen::VectorXd w_` is contradictory. The `state()` implementation (`x_hat = Sigma w_i * x_i`) confirms that raw normalised probability weights are stored, not log-space values.
...
`K_MSEC(period_ns_ / 1'000'000ULL)` rounds any period below 1 ms to `K_MSEC(0)`, which creates an immediate one-shot Zephyr timer rather than a periodic one. A Ts of 500 mus - perfectly normal for fast inner control loops - silently produces wrong scheduling behaviour.
...
| 7 | HIGH | High | `lib/NonlinearMPC.cpp:115` | Performance | Per-step O(Np*n^2) allocation in `buildAndSolve()` |
...
| 67 | LOW | `ExtremumSeeker` has no convergence test | `tests/test_catch2_advanced.cpp` | Added `"ExtremumSeeker converges to minimum of quadratic J(theta)=(theta-2)^2"` `[extremum_seeker]` test: 20 000 steps of the static cost surface `J = (u-2)^2`; asserts `currentEstimate()` within 0.5 of 2.0. |
...
| 42 | MED | Verified closed | `[zpetc]` amplitude-flatness test already present at `test_catch2_advanced.cpp:5533` (ZPETC gives pure delay z^{-d}, not zero phase; test correctly checks `|G.Gff| approx = 1`) |
