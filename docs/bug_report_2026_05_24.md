...
For `Nu=5`, `m=1`, this is a 5*5 factorisation - tolerable offline. For `Nu=20`, `m=4`, it's an 80*80 LDLT every step, which is O(n^3/3) approx = 170k FLOPs/step you're throwing away.
...
This is `20 * (1 + 0.3) = 26.0`. The intent was to initialise the state so that `C*x = 20^\circC` at t=0 (steady state at reference). But `C = [0, 1]`, so the output is `x(1)` directly. Setting `x(1) = 26` means the simulation starts with the measured temperature at 26^\circC, not 20^\circC. There's also a dangling `* 0.0` multiplied on the output of `ssStep` for the fuzzy branch (line 95), which zeros out an additive correction that was presumably left over from debugging.
...
The reference reconstruction `r = y_hat + error = (C*x + D*u_prev) + (r - y_true)` assumes `y_hat approx = y_true`, which is valid only when `D = 0`. When `D != 0`, `y_hat` uses `u_prev_` (u[k-1]) while `y_true` in the error uses `u[k]` (implicit in the feedback loop). The header comment acknowledges this and says to use `computeRef()` directly, but the warning is easy to miss. Add a `std::cerr` warning in the constructor when `D.norm() > 1e-12` to make it discoverable.
...
**Severity:** Medium - unnecessary O(Np.Nc.n^2) work on every Q/R weight update  
**Description:**  
...
| **P1** | ex23 wrong warm-start IC (output starts at 26^\circC not 20^\circC) | `examples/ex23_fuzzy_pd_temperature.cpp` | Small |
