...
| Code generation (C/C++ from controller design) | `lib/CodeGenC.h`/`.cpp` - flat, dependency-free C99 emission (no class/inheritance, matching the target's one-controller-one-corrector MCU constraint) for a single tuned `DiscretePID`/`DiscreteSMC` (first-order)/`DiscreteLeadLag` ("step-based": single-pass, O(1), no internal loop), with an optional `AntiWindupWrapper`-equivalent corrector fused inline - Phase 4 DT1. `FuzzyPD`/`FuzzyPID` (iterative CoG grid search per step), `DiscreteMPC` (iterative QP solve per step), `float` emission, and Python bindings for the generator are explicitly out of scope for CPU-cycle/memory predictability on constrained MCU targets; see the design doc. |
