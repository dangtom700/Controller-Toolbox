...
RLS update (exponential forgetting, factor lambda \in (0,1]):

```
K[k]      = P[k-1] phi[k] / (lambda + phi[k]ᵀ P[k-1] phi[k])
theta^[k]     = theta^[k-1] + K[k] (y[k] - phi[k]ᵀ theta^[k-1])
P[k]      = (I - K[k] phi[k]ᵀ) P[k-1] / lambda
```

The identified theta^ is converted to a `StateSpace` and fed to `GPC::setPlant()`, closing the adaptive loop.
...
A scheduling variable rho (e.g., airspeed, temperature, load) selects or interpolates among pre-designed gain tables:

```
K(rho) = Sigma_i w_i(rho) K_i,   Sigma w_i = 1,   w_i >= 0
```

**Toolbox mapping:** `ControllerStack::Weighted` mode with `w_i` as runtime weights, or direct matrix blending `K_blend = (1-rho)*K_slow + rho*K_fast` (see `ex41_lpv_gain_scheduling.cpp`).
...
u = thetaᵀ(t) omega                (parameterised control law)
theta. = -Gamma e_m Pᵀ B_m omega       (MIT/Lyapunov gradient adaptation)
```
...
### A2. Robust Control - Hinf, mu-Synthesis, QFT

**Core idea.** Guarantee a worst-case performance bound for all plants in a norm-bounded uncertainty set.

#### 2.1 Hinf Synthesis

Mixed-sensitivity formulation: choose weighting functions W1 (sensitivity), W2 (control effort), W3 (complementary sensitivity) and solve:
...
where S = (I + GK)^-^1, T = GK(I + GK)^-^1. Solved by gamma-bisection via Hamiltonian or discrete Riccati equations.

**Toolbox mapping:**

| Method | Call | File |
|--------|------|------|
| Direct Hinf | `DiscreteHinf::solve(plant, W1, W2, W3)` | `lib/DiscreteHinf.h` |
...
**mu-synthesis DK-iteration** (implemented in Part 18): at each iteration, scale the plant uncertainty channels by diagonal D matrices to minimise mu(Delta) upper bound, then re-solve Hinf. With `MuSynParams::useRationalD = true`, first-order rational D_j(z) filters are fitted per channel:

```
D_j(z) = K . (z - z0) / (z - p),   p = 0.85
```
...
```
J = Sigma_{k=0}^{inf} (xₖᵀ Q xₖ + uₖᵀ R uₖ)
```

Solved offline via DARE -> gain K = (R + BᵀPB)^-^1BᵀPA. Online cost: one matrix-vector multiply per step.
...
```
min_{DeltaU} ½ DeltaUᵀ H DeltaU + fᵀ DeltaU
s.t.     u_min <= u_prev + Gamma DeltaU <= u_max
         Deltau_min <= DeltaU <= Deltau_max
```

Condensed prediction: Y = F x + G_u u_prev + Phi DeltaU. Solved via `GradientProjectionQP` (shared backend with GPC and MHE).
...
Mamdani: rule firing strength `alpha_i = min(mu_A(e), mu_B(ė))` -> aggregated output clipped MF -> CoG defuzzification.
...
```
trigger[k] = 1  iff  ||x[k] - x[last]|| > delta
```

**Self-triggered schedule:** next update at k* = argmin{k > k_last : V(x[k]) >= gamma . V(x[k_last])} for a Lyapunov function V.
...
Control: `u[k] = -K . sat(s[k] / phi)` - saturation replaces discontinuous sign to limit chattering.

`c_de` absorbs Ts: to match continuous-time slope lambda [rad/s], set `c_de = lambda . Ts`.
...
- Full nonlinear state equations `xdot = f(x) + g(x)u` in symbolic or functional form.
...
**Formal model:**
```
q[k+1] = delta(q[k], x[k], r[k])        (discrete state transition)
u[k]   = f_{q[k]}(x[k], r[k])       (mode-specific continuous control)
```

Stability condition (Lyapunov): there exists a common Lyapunov function V(x) such that V decreases along trajectories of each mode, or dwell-time condition T_d >= tau_d is satisfied.
...
1. Solve the discrete Lyapunov equations for controllability gramian P_c and observability gramian P_o:
   ...
   Aᵀ P_o A + Cᵀ C = P_o
   ```
2. Compute balanced realisation: `T = P_c^{1/2} U Sigma^{-1/2}` where `P_c^{1/2} P_o P_c^{1/2} = U Sigma^2 Uᵀ`.
3. Truncate states with Hankel singular values `sigma_i < epsilon_tol`.

**Error bound (Hinf):** `||G - G_r||inf <= 2 Sigma_i=ᵣ₊1^n sigma_i` - provides a-priori guarantee on approximation quality.

**Simplifies controller design:** Reduces a 50-state flexible structure to a 4-state rigid-body + first-flex-mode model -> enables PID or LQR on the reduced plant, with the truncation error treated as Hinf uncertainty in a robust design.
...
**What it does.** Approximates the nonlinear plant `xdot = f(x,u)` by its Jacobian at an operating point (x0, u0):

```
A = df/dx |_{x0,u0},   B = df/du |_{x0,u0}
```

enabling design of LQR, MPC, or Hinf on the linear model.
...
```
xdot1 = f1(x1) + g1(x1) x2
xdot2 = f2(x1,x2) + g2(x1,x2) x3
⋮
xdot_n = f_n(x) + g_n(x) u
```

Design Lyapunov functions `V1, V2, ..., V_n` recursively. At each step, the next state `x_i₊1` acts as a virtual control for the `x_i` subsystem. The final control law `u` drives the last virtual tracking error to zero.
...
**Feasibility condition:** Each `g_i(x) != 0` for all reachable x (non-singular virtual actuators).
...
```
u = u_ES + u_DI = g\perp(x) - kd xdot_output
```

where g\perp is the interconnection matrix for energy shaping and kd > 0 is the damping gain.
...
```
d^(z) = Q(z) . [y(z) - G_nom(z) . u(z)]
u(z)  = u_primary(z) - d^(z)
```

Q(z) is a low-pass filter (bandwidth omega_Q below plant bandwidth). The effective plant seen by the primary controller is G_nom(z) for all disturbances below omega_Q.

**ADRC ESO (3-state for 2nd-order plant):**

```
[ẑ1]   [1-beta1Ts    Ts    Ts.Ts/2] [ẑ1]   [beta1Ts] [  0  ]
[ẑ2] = [  -beta2Ts   1      Ts   ] [ẑ2] + [beta2Ts] [b0.Ts] u
[ẑ3]   [  -beta3Ts   0      1   ] [ẑ3]   [beta3Ts] [  0  ]

u = (r_dd - ẑ2.omega_c^2 - ẑ3) / b0   [PD + total-disturbance cancellation]
```

**Simplifies:** The primary controller is designed for the nominal model G_nom only. All disturbances, model errors, and nonlinearities below omega_Q are handled transparently.
...
MPC is not strictly a "model interference" technique, but it is the most model-dependent control strategy in the toolbox. Its condensed prediction matrices F, G_u, Phi are rebuilt from the plant model at construction or on `setPlant()` call.
...
**What it does.** For a minimum-phase plant G(z) (all zeros strictly inside the unit circle), compute the stable inverse G^-^1(z) and apply it to the reference signal:

```
u_ff(z) = G^-^1(z) . r(z)   ->   y(z) approx = r(z) after the plant delay
```

The feedback controller only needs to reject disturbances and compensate model error - not drive tracking.

**Feasibility condition:** All zeros of G(z) must satisfy |z_i| < 1 (minimum phase). For non-minimum-phase plants, the inverse is unstable - a partial (causal) inversion or zero-phase preview filter is needed instead.
...
```
DeltaU[n+1] = Pi_{𝒞}(DeltaU[n] - alpha . nablaJ(DeltaU[n]))
```

where `Pi_{𝒞}` projects onto the box constraint set 𝒞.
...
|------|-----------|------------|
| Chattering | High-frequency switching at sensor noise level | Saturation boundary layer (phi > 0); super-twisting |
| Actuator wear | Rapid sign reversals in u | Increase phi; filter u through low-pass |
...
| Surface non-invariance | Discretisation of sign function allows limit cycles | Use `c_de = lambda.Ts` correctly (see deployment.md) |
...
**Toolbox:** `DiscreteSMC` in `lib/DiscreteSMC.h`. Parameter pitfall: `SMCParams::c_de` must absorb Ts (`c_de = lambda.Ts`), not the continuous-time slope lambda directly. See `test_catch2_advanced.cpp [smc]`.
...
```
u = (v - f(x)) / g(x)   ->   xdot = v   (linear double-integrator)
```
...
| Risk | Mechanism | Mitigation |
|------|-----------|------------|
| Model error -> instability | Perfect cancellation of f(x) depends on exact model; residual error enters as unmatched disturbance | Robust FL: add SMC or Hinf outer loop |
...
```
e_m = y - y_m  ->  0   as t -> inf
```

The adaptation law (MIT rule / Lyapunov gradient):

```
theta.(t) = -gamma . e_m(t) . dy/dtheta      (MIT rule)
theta.(t) = -Gamma . e_m . Pᵀ B_m omega       (Lyapunov-based, guaranteed stable)
```
...
3. **Persistent excitation** of the reference r(t) - without PE, parameter convergence is not guaranteed even if tracking error converges. Consequence: the parameter estimates theta may stagnate away from true values, creating a dormant instability.
4. High-gain adaptation (large Gamma) -> fast tracking but risk of high-frequency oscillation.

**Real-world risks:**

| Risk | Mechanism | Mitigation |
|------|-----------|------------|
| Bursting / transient instability | Slow adaptation + unmodelled dynamics -> temporary divergence | Add a deadzone ||e_m|| < delta; use sigma-modification |
| PE requirement | Constant reference -> zero gradient -> parameter drift | Inject small PRBS dither on reference |
| Control saturation | Adaptation may demand large u temporarily | Bound theta; add projection operator to keep theta in known feasible set |
...
**Do NOT use:** Plants with unstable zeros, rapidly changing plants (faster than the adaptation rate Gamma), or when exact y_m tracking carries safety-critical timing constraints that cannot tolerate transient bursting.
...
```
u(t) = theta(t) + a . sin(omegaₛ t)         [dithered input]
J_filtered = LPF{J(y(t))}
nabla^J = HPF{J(y(t))} . sin(omegaₛ t)       [demodulated gradient]
theta. = -k . nabla^J                         [integrator]
```

The ESC asymptotically converges to a neighbourhood of size O(a^2) around the true extremum - the dither cannot be removed without losing gradient information.

**Feasibility conditions:**

1. J(theta) has a unique local minimum (or maximum) in the operating region.
2. Plant dynamics G(s) are much faster than the dither frequency omegaₛ (quasi-static approximation). Rule of thumb: omegaₛ < omega_bandwidth / 10.
...
| Risk | Mechanism | Mitigation |
|------|-----------|------------|
| Persistent oscillation at optimum | Dither is never removed - it is the sensing mechanism | Set a << operating range; accept O(a^2) neighbourhood |
| Slow convergence | Integrator gain k small for stability, but this slows learning | Tune omegaₛ and k jointly: omegaₛ approx = 10.k for separability |
...
| Interaction with closed-loop poles | If omegaₛ is near a closed-loop resonance, the loop can amplify dither | Check open-loop frequency response at omegaₛ |
...
Deltau_min <= Deltau[k] <= Deltau_max  (rate of change limits)
...
| LDLT failure | `rho_u` too small -> ill-conditioned Hessian | Floor: `rho_u >= 1e-6 * rho_y * max_eig(Phiᵀ Phi)` |
...
**Intended effect.** Pre-filter the reference r[k] through the stable inverse G^-^1(z) so the output y[k] tracks r[k] with zero steady-state error and minimal transient error, without requiring the feedback controller to drive the tracking:

```
u_ff[k] = G^-^1(z) . r[k]   ->   y[k] = G(z) . u_ff[k] = r[k]   (exact, no delay)
```
...
**Feasibility condition:** All zeros of G(z) must satisfy |z_i| < 1 (minimum phase). A non-minimum-phase zero at z0 with |z0| > 1 creates an unstable pole in G^-^1(z) -> the feedforward diverges for any non-trivial reference.
...
| Risk | Mechanism | Mitigation |
|------|-----------|------------|
| Zeros near unit circle | G^-^1 has poles near unit circle -> near-resonance amplification | Regularise: `G_ff = G^-^1 . (1 + epsilon.G^-^1)^-^1` |
| Noise amplification | G^-^1 is high-pass for typical low-pass plants | Pre-filter r[k] with a reference governor |
...
| Structured parametric uncertainty (quantified) | Hinf / mu-synthesis | `DiscreteHinf::solveMuSyn()` | Conservative (gamma larger than needed) |
...
|- NO: What is the primary obstacle?
       ...
       |   |- If disturbance is unknown, plant approx = integrator -> DiscreteADRC (ESO)
       ...
       |   |- Hinf (DiscreteHinf::solve) or mu (solveMuSyn)
...
struct TruncationResult {
    ...
    double      errorBound;           // 2 * sum(sigma_{r+1}..sigma_n) [Hinf norm]
};
...
- All operations are O(n^3) offline - not in the control loop.
...
**Motivation.** MRAC is the most widely taught adaptive control method but is absent from the toolbox. The Lyapunov-based version (with sigma-modification for robustness) is implementable with the existing Eigen infrastructure.
...
struct MRACParams {
    ...
    double     sigma;           // sigma-modification gain (0 = off, ~0.01 recommended)
...
public:
    ...
    void   reset() override;
    VectorXd parameters() const;            // current theta^
...
**Implementation notes:**
- Lyapunov adaptation: `theta_[k] = theta_[k-1] + Ts * (-Gamma * em * phi - sigma * theta_)` where `phi` is the regressor vector and sigma-modification prevents parameter drift.
...
// Plant: xdot = -x^3 + u (strongly nonlinear, relative degree 1, minimum phase)
// Linearised: xdot = v with inner PID Kp=5, Ki=2
...
- Partition roots into |z_i| < 1 (B+) and |z_i| >= 1 (B-).
- Normalise so DC gain = 1 (divide by B-(1)^2 ).
...
```cpp
// [zpetc] tag  
// Plant: G(z) = (z - 1.2) / (z^2 - 1.5z + 0.56)  (NMP zero at z=1.2)
// REQUIRE: hasNMPZeros == true
// REQUIRE: phase of (G_ff . G) at omega=0.1.pi/Ts is within 0.1^\circ of 0^\circ
