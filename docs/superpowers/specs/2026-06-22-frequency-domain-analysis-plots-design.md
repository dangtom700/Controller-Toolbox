...
|---|---|---|
| Bode (mag/phase vs omega) | `SystemAnalysis.get_frequency_response` (existing, SISO) | None - pure Python |
...
| Singular-value plot (MIMO, sigma vs omega) | new `SystemAnalysis::getSingularValues(sys, freqs)` | ~60-80 lines C++ - `get_frequency_response` is explicitly SISO-only |
...
1. **`lib/SystemAnalysis.h` / `.cpp`** - add
   ...
   ```
   Evaluates the full `G(e^{jomegaTs}) = C(zI-A)^-1 B + D` matrix (not just the SISO scalar
