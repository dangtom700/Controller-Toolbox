"""
Detect and report the status of every C++ and Python case study under case-study/.

Scope: C++ and Python studies only -- the ones run.py builds and runs (Phase 5 /
Phase 7). Two kinds of directory are skipped: common/ (shared header-only tooling,
not a study) and MATLAB-native studies, which are run by hand via run_all.m and have
no sim/ framework to score. See is_matlab_study() for the rationale and detection.
...
MATLAB_EXTENSION = {'.m', '.mlx', '.slx', '.mdl'}


# -- Scope -----------------------------------------------------------------------

def is_matlab_study(case_study_path: pathlib.Path) -> bool:
    """True for MATLAB-native studies, which are deliberately OUT OF SCOPE here.

    This tracker reports only C++ and Python studies -- the ones `run.py` builds
    and runs (Phase 5 / Phase 7). MATLAB studies (e.g. `Boiler Control MATLAB`)
    are driven by hand via `run_all.m` against the MATLAB toolboxes, are not part
    of any `run.py` phase, and have no `sim/` framework, so every axis this
    tracker reports on -- language, 4-tier status, placeholder-scaffold
    detection -- is meaningless for them. Left in, such a study is scored against
    a contract it was never meant to satisfy and lands as `undetermined` /
    `Not started` despite being complete.

    Detection is by shape, not by name, so a second MATLAB study is excluded
    automatically: no `sim/` framework, plus either a `matlab/` directory or any
    MATLAB source file in the tree.
    """
    if _has_sim(case_study_path):
        return False
    if (case_study_path / "matlab").is_dir():
        return True
    return any(f.suffix.lower() in MATLAB_EXTENSION
               for f in case_study_path.rglob("*") if f.is_file())
...
def main() -> None:
    ...
    for name in sorted(ROOT.iterdir(), key=lambda p: p.name.lower()):
        ...
        if not name.is_dir() or name.name == "__pycache__" or name.name == "common":
            continue
        # MATLAB-native studies are out of scope -- see is_matlab_study().
        if is_matlab_study(name):
            skipped_matlab.append(name.name)
            continue
    ...
    auto_message = (
        ...
        "> Scope: C++ and Python studies only (the ones `run.py` builds and runs).\n"
        "> MATLAB-native studies are tracked by hand and excluded here"
        + (" - currently: " + ", ".join(f"`{n}`" for n in skipped_matlab) if skipped_matlab else "")
        + ".\n\n"
    )
    ...
    if skipped_matlab:
        print(f"Skipped (MATLAB-native, out of scope): {', '.join(skipped_matlab)}")
